SVAR2 <- function (var, data, estmethod = c("scoring", "direct"), Amat = NULL, 
                   ...
                   maxls = 1, lrtest = TRUE, ...) 
{
  ...
  if (estmethod == "scoring") {
    ...
    r <- c(ra, rb)
    Kkk <- diag(Ksq)[, c(sapply(1:K, function(i) seq(i, 
                                                     Ksq, K)))]
    IK2 <- diag(Ksq)
    IK <- diag(K)
    iters <- 0
